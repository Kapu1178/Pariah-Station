...
/mob/living/proc/Disorient(amount, stamina_amount, ignore_canstun, knockdown, stun, paralyze, overstam, stack_status)
	...
	if(stamloss >= 40)
		set_confusion(min(get_confusion() + (amount / 10), 10))
	...
	var/datum/status_effect/incapacitating/disoriented/D = IsDisoriented(FALSE)
	if(D)
		D.duration = min(15 SECONDS, amount + D.duration)
	else if(amount > 0)
		D = apply_status_effect(/datum/status_effect/incapacitating/disoriented, amount)
